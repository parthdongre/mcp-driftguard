...
def main() -> None:
    ...
    fold_payloads = []
    by_strategy: dict[str, list[float]] = {strategy.value: [] for strategy in TemporalStrategy}

    for fold in folds:
        evaluations = evaluate_attack_family_holdout(fold)
        for result in evaluations:
            by_strategy[result.strategy.value].append(
                result.strict_unseen_family_metrics.post_onset_block_rate
            )
    ...
    summary = {
        strategy: {
            "mean_post_onset_block_rate": round(mean(values), 6),
            "worst_family_post_onset_block_rate": round(min(values), 6),
        }
    ...
    payload = {
        ...
        "strict_metric_note": (
            "post_onset_block_rate gives no credit for a block before the held-out family "
            "appears. pre_onset_block_rate is reported separately to expose apparent wins "
            "caused by the shared capability-expansion setup."
        ),
        "summary": summary,
        "folds": fold_payloads,
    }
